...
export class PropertyComponent implements OnInit {
  ...
  getActivity(id) {
    this._pageServices.getactivity(id).subscribe(res => {
  ...
  // aggrementCountInDays(startDate,endDate) {
  ...
  getPropertyTypeController() {
    this._pageServices.getPropertyTypeController().pipe(takeUntil(this.notifier)).subscribe((res: any) => {
      ...
      this.sub = this.activatedRoute.params.subscribe(params => {
        // this.getPropertyType(params['id']);
        this.getTenentData();
  ...
  getTenentData(){
    this._pageServices.getTenentData().subscribe(res => {
      this.tenentData = res.data;
    })
  }

  getTenentFromOwenrId(id){
     const tenent =  this.tenentData?.filter(element => element.owner.owner_id == id );
    return tenent;
  }
}